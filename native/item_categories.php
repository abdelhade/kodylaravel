                ...
                <div class="table-responsive" id="">
                    <table>
                        ...
                        <tbody>
                            <?php 
                            $x=0;
                            $resgrb =$conn->query("SELECT * FROM item_group2 where isdeleted = 0");
                            while ($rowgrb = $resgrb->fetch_assoc()) {
                            $x++;
                            ?>
                            <form action="do/doedit_group2.php?id=<?= $rowgrb['id']?>" method="post">
                            <tr>
                                <th><?= $x ?></th>
                                <th><input type="text" value="<?= $rowgrb['gname']?>" name="gname"></th>
                                <th>
                                    <button type="submit" class="btn btn-sm btn-warning">
                                    <i class="fa fa-pen"></i></button>
                                <a href="do/dodel_group2.php?id=<?= $rowgrb['id'] ?>" class="btn bg-red-600 text-red-100"><i class="fa fa-trash"></i></a>
                                </th>
                            </tr>
                            </form>
                            <?php }?>
