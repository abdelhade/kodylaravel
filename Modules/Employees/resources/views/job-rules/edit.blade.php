        ...
        <div class="container-fluid">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">{{ $lang['lang_edit'] ?? 'تعديل قاعدة وظيفة' }}</h3>
                </div>
                <form action="{{ route('job-rules.update', ['id' => $rule->id]) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        @if($errors->any())
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="info" id="info" cols="20" rows="5">{{ old('info', $rule->info) }}</textarea>
                        </div>
                    </div>
