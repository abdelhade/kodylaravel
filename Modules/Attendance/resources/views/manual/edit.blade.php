        ...
        <div class="container-fluid">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">{{ $lang['lang_adfp_add'] ?? 'تعديل بصمة' }}</h3>
                </div>
                <form role="form" action="{{ route('manual-attendance.update', ['id' => $attendance->id]) }}" method="post">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group col-md-3">
                            ...
                            <select class="form-control" name="employee" id="emp" required>
                                @foreach($employees as $emp)
                                    <option value="{{ $emp->id }}" {{ old('employee', $attendance->employee) == $emp->id ? 'selected' : '' }}>
                                        {{ $emp->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control col-lg-2" type="date" name="fpdate" id="fpdate" value="{{ old('fpdate', $attendance->fpdate) }}" required>
                        </div>

                        <div class="form-group">
                            <label for="fptime">{{ $lang['lang_adfp_time'] ?? 'الوقت' }}</label>
                            <input class="form-control col-lg-2" type="time" name="fptime" id="fptime" value="{{ old('fptime', $attendance->time) }}" required>
                        </div>
                    </div>
