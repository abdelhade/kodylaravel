...
class SalesController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // فقط للفواتير الفعلية (مبيعات ومردودات)
            if (in_array($pro_tybe, [self::INVOICE_TYPES['SALES'], self::INVOICE_TYPES['POS'], self::INVOICE_TYPES['SALES_RETURN']])) {
                // الحصول على رقم القيد التالي
                $maxJournalId = DB::table('journal_heads')->max('journal_id') ?? 0;
                $journalId = $maxJournalId + 1;
                
                // تحديد نوع القيد
                $journalDetails = $pro_tybe == self::INVOICE_TYPES['SALES_RETURN'] 
                    ? 'مردود مبيعات _ ' . $last_op 
                    : 'فاتورة مبيعات _ ' . $last_op;
                ...
                if ($pro_tybe == self::INVOICE_TYPES['SALES_RETURN']) {
                    // مردود مبيعات: عكس القيد
                    // من حـ/ المخزن (مدين)
