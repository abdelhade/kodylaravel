    ...
    <div class="container-fluid p-2">
        ...
        <div class="card" style="border-radius: 0 0 10px 10px;box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <div class="card-body p-3">
                <form action="{{ $is_edit_mode ? route('sales.update', $invoice_data->id ?? 0) : route('sales.store') }}"
                    method="POST" id="salesForm">
                    @csrf
                    @if ($is_edit_mode)
                        @method('PUT')
                    @endif
                    ...
                    <div class="row mb-3" style="background: #f5f5f5;padding: 15px;border-radius: 8px;">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="acc2_id" class="form-control" required style="border: 2px solid #26a69a;">
                                ...
                                @foreach (DB::table('acc_head')->where('isdeleted', 0)->get() as $client)
                                    <option value="{{ $client->id }}"
                                        {{ $is_edit_mode && $invoice_data->acc2 == $client->id ? 'selected' : '' }}>
                                        {{ $client->aname }}
                                    </option>
                                @endforeach
                            </select>
                                ...
                                @if ($is_edit_mode)
                                    @foreach (DB::table('fat_details')->where('pro_id', $invoice_data->id)->where('isdeleted', 0)->get() as $index => $detail)
                                        <tr>
                                            ...
                                            <td><input type="text" class="form-control" value="وحدة" readonly></td>
                                            <td><input type="number" name="itmqty[]" class="form-control item-qty"
                                                    value="{{ $detail->qty_out }}" step="0.01" required></td>
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-6">
                            <div style="background: #f5f5f5;padding: 20px;border-radius: 8px;">
                                ...
                                <div class="row mb-2">
                                    <div class="col-3 " style='direction:rtl '><strong>الخصم:</strong></div>
                                    <div class="col-8">
                                ...
                                <div class="row mt-2">
                                    <div class="col-3 " style='direction:rtl '><strong style="color: #d32f2f;">الباقي:</strong></div>
                                    <div class="col-8">
