...
@section('content')
                        ...
                        <form action="{{ route('activities.update') }}?id={{ $activity->id }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('related_to') is-invalid @enderror" 
                                    ...
                                    <option value="">اختر النوع</option>
                                    <option value="lead" {{ old('related_to', $activity->related_to) == 'lead' ? 'selected' : '' }}>عميل محتمل</option>
                                    <option value="contact" {{ old('related_to', $activity->related_to) == 'contact' ? 'selected' : '' }}>جهة اتصال</option>
                                    <option value="opportunity" {{ old('related_to', $activity->related_to) == 'opportunity' ? 'selected' : '' }}>فرصة</option>
                                </select>
                                @error('related_to')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="form-group">
                                ...
                                <input type="datetime-local" class="form-control @error('activity_date') is-invalid @enderror" 
                                       ...
                                       value="{{ old('activity_date', date('Y-m-d\TH:i', strtotime($activity->activity_date))) }}" required>
                                @error('activity_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('status') is-invalid @enderror" 
                                    ...
                                    <option value="planned" {{ old('status', $activity->status) == 'planned' ? 'selected' : '' }}>مخطط</option>
    ...
    <script>
        const currentRelatedTo = '{{ $activity->related_to }}';
        const currentRelatedId = '{{ $activity->related_id }}';
        
        function loadRelatedOptions() {
            const relatedTo = document.getElementById('related_to').value;
            const relatedId = document.getElementById('related_id');
            relatedId.innerHTML = '<option value="">اختر الجهة</option>';
            
            if (relatedTo === 'lead') {
                @foreach($leads as $lead)
                    const selected = currentRelatedId == '{{ $lead->id }}' ? 'selected' : '';
                    relatedId.innerHTML += '<option value="{{ $lead->id }}" ' + selected + '>{{ $lead->name }}</option>';
                @endforeach
            } else if (relatedTo === 'contact') {
                @foreach($contacts as $contact)
                    const selected = currentRelatedId == '{{ $contact->id }}' ? 'selected' : '';
                    relatedId.innerHTML += '<option value="{{ $contact->id }}" ' + selected + '>{{ $contact->name }}</option>';
                @endforeach
            }
        }

        document.getElementById('related_to').addEventListener('change', loadRelatedOptions);
