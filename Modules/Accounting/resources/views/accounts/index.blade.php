...
<style>
    ...
    /* Tree View Styles */
    #tree-container ul { list-style: none; padding-right: 25px; }
    .caret { cursor: pointer; user-select: none; font-size: 1.1rem; }
    .caret::before { content: "\f0d9"; font-family: "Font Awesome 5 Free"; font-weight: 900; margin-left: 8px; transition: 0.3s; display: inline-block; }
    .caret-down::before { transform: rotate(-90deg); }
    .nested { display: none; margin-top: 10px; }
    .active-node { display: block; }
    .tree-item { padding: 8px 0; border-bottom: 1px dashed #e2e8f0; }
</style>
        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <h1 class="page-title">
                    ...
                    @if($accType)
                        <span class="badge badge-light ml-2 text-muted" style="font-size: 1rem;">{{ $accounts->count() }}</span>
                    @endif
                </h1>
            </div>
            <div class="col-md-6 d-flex justify-content-end gap-3">
                ...
                @if($accType)
                    <a href="{{ route('accounts.index') }}" class="btn btn-outline-secondary" style="border-radius: 10px; padding: 12px 20px;">
                ...
                <div class="table-card p-4 mb-4" id="tree-container">
                    ...
                    <ul id="myUL">
                        @foreach($treeAccounts as $account)
                            @include('accounting::accounts.partials.tree-item', ['account' => $account, 'level' => 0])
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            <table class="premium-table" id="accountsTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if($accType)
                            <td>
                                <span class="{{ ($account->balance ?? 0) >= 0 ? 'balance-positive' : 'balance-negative' }}">
                        ...
                        @else
                            <td>{{ $account->kind == 1 ? 'ميزانية' : 'أرباح وخسائر' }}</td>
                            <td>
                                @if($account->is_basic)
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                <a href="{{ route('accounts.edit', ['id' => $account->id, 'acc' => $accType]) }}" class="btn btn-sm btn-light border text-primary" title="تعديل">
                                    <i class="fas fa-edit"></i>
                                </a>
                                @if(!$account->is_basic)
                                    <form action="{{ route('accounts.destroy') }}" method="GET" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا الحساب؟');">
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ $accType ? 6 : 5 }}" class="text-center py-5">
